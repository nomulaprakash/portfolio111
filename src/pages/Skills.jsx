...
const skills = [
  ...
  { name: 'Python', icon: <SiPython className="text-yellow-400" /> }, // ✅ Fixed this line
...
const Skills = () => {
  return (
    <section
      id="skills"
      className="min-h-screen flex flex-col justify-center items-center bg-transparent text-white py-16 px-4"
    >
      <h2 className="text-3xl md:text-5xl font-bold text-center mb-12">My Skills</h2>

      {/* Skills Grid */}
      <div className="w-full max-w-6xl grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mb-16">
      ...
      <h2 className="text-3xl md:text-5xl font-bold text-center mb-12">My Certificates</h2>
      <div className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {certificates.map((cert, index) => (
          <a
            ...
            className="bg-white/10 border border-white/20 rounded-xl overflow-hidden hover:shadow-lg transition"
          >
            <img
              src={cert.img}
              alt={cert.title}
